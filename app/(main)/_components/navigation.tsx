...
export const Navigation = () => {
  ...
  const handleMouseMove = (event: MouseEvent) => {
    ...
    if (sidebarRef.current && navbarRef.current) {
      ...
      navbarRef.current.style.setProperty("left", `calc(100% - ${newWidth}px)`);
    }
  };
  ...
  return (
    <>
      <aside ref={sidebarRef} className={cn("group/sidebar h-full bg-secondary overflow-y-auto relative flex flex-col w-60 z-[99999]", isResetting && "transition-all ease-in-out duration-300", isMobile && "w-0")}>
        <div
          onClick={collapse}
          role="button"
          className={cn("h-6 w-6 to-muted-foreground rounded-sm hover:bg-neutral-300 dark:hover:bg-neutral-600 absolute top-3 right-2 opacity-0 group-hover/sidebar:opacity-100 transition", isMobile && "opacity-100")}
        >
